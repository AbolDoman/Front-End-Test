...
export default function Register() {
    ...
    const validate = (data) => {
        ...
        if(!data.confPassword){
            if(str)
                str += ' , ';
            str = str + "Confrim Password is required!\n";
        }
        if(str)
            return(str);
        else{
            if(data.password !== data.confPassword)
                str += "Conf Password and Password is not equal\n";
            return(str)
        }
    };
    const handleSubmit = (event) => {
        ...
        if(error){
            ...
            //         localStorage.setItem("image",response.image);
                     localStorage.setItem("x-auth-token","jhbyt6789");
            ...
            <Box sx={{
                ...
                <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
                    ...
                    <TextField
                        ...
                        id="confPassword" />
                    <FormControlLabel control={<Checkbox value="remember" color="warning" />} label="remember"/>
                    <Button type="submit" fullWidth variant="contained" color={"warning"} sx={{ mt: 3, mb: 2 }}>register</Button>
                </Box>
            </Box>
        </Container>
    );
